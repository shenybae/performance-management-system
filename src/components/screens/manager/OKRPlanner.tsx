    ...
    return parsed.map((item: any, index: number) => ({
      ...
      proof_files: Array.isArray(item?.proof_files)
        ? item.proof_files
            ...
            .filter((file: GoalProofFile) => !!file.proof_file_data)
        : [],
...
export const OKRPlanner = ({ employees }: OKRPlannerProps) => {
  ...
  const [memberRatingsModal, setMemberRatingsModal] = useState<{ open: boolean; goalId: number | null; ratings: Record<string, number>; submitting: boolean }>({ open: false, goalId: null, ratings: {}, submitting: false });
  const [inlineMemberRatingsByGoal, setInlineMemberRatingsByGoal] = useState<Record<number, Record<string, number>>>({});
  const [inlineMemberRatingsSavingGoalId, setInlineMemberRatingsSavingGoalId] = useState<number | null>(null);
  ...
  const reviewGoalFinalProof = async (
    ...
    const isGoalAlreadyApproved = String(proofReviewGoal?.proof_review_status || '').trim() === 'Approved';
    const goalReviewedRole = String((proofReviewGoal as any)?.proof_reviewer_role || (proofReviewGoal as any)?.proof_reviewed_role || '').trim().toLowerCase();
    const hasLockedGoalRating = Number(proofReviewGoal?.proof_review_rating || 0) > 0;
    if (isGoalAlreadyApproved && goalReviewedRole === 'manager' && hasLockedGoalRating) {
      window.notify?.('Final goal proof rating is already locked', 'error');
      return;
    }

    if (status === 'Approved' && !allSubmittedProofsApproved) {
      window.notify?.('Approve all submitted delegated task proofs first before final goal approval', 'error');
      return;
    }
  ...
  const openMemberRatingsModal = (goalId: number) => {
    ...
    // Add ratings for each member who has an approved task proof
    submittedTasks.forEach((t: any) => {
      const memberId = `task-${t.id}`;
      initialRatings[memberId] = Number(t.proof_review_rating || 0);
    });

    // Add rating for goal leader (final goal proof)
    const goalReviewRole = String((goal as any).proof_reviewer_role || (goal as any).proof_reviewed_role || '').trim().toLowerCase();
    if (goal.proof_review_status === 'Approved' && goalReviewRole === 'manager') {
      initialRatings['leader'] = Number(goal.proof_review_rating || 0);
    }

    setMemberRatingsModal({
      open: true,
      goalId,
  ...
  const saveMemberRatings = async () => {
    const goalId = memberRatingsModal.goalId;
    if (!goalId) return;

    setMemberRatingsModal(prev => ({ ...prev, submitting: true }));
    try {
      const goal = goals.find(g => Number(g.id) === Number(goalId));
      const tasks = proofReviewTasksByGoal[goalId] || [];
      const submittedTasks = tasks.filter((t: any) => {
      ...
      // Save rating for goal leader
      if (goal && memberRatingsModal.ratings['leader'] && memberRatingsModal.ratings['leader'] > 0 && memberRatingsModal.ratings['leader'] <= 5) {
        savePromises.push(
          fetch(`/api/goals/${goalId}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
            body: JSON.stringify({ proof_review_rating: memberRatingsModal.ratings['leader'] })
          })
        );
      }
      ...
      window.notify?.(e?.message || 'Failed to save member ratings', 'error');
    } finally {
      setMemberRatingsModal(prev => ({ ...prev, submitting: false }));
    }
  };

  const buildInitialRatingsForGoal = (goalId: number) => {
    const goal = goals.find(g => Number(g.id) === Number(goalId));
    const tasks = proofReviewTasksByGoal[goalId] || [];
    const submittedTasks = tasks.filter((t: any) => {
      const files = parseTaskProofFiles(t);
    ...
    const initialRatings: Record<string, number> = {};
    submittedTasks.forEach((t: any) => {
      initialRatings[`task-${t.id}`] = Number(t.proof_review_rating || 0);
    });

    const goalReviewRole = String((goal as any)?.proof_reviewer_role || (goal as any)?.proof_reviewed_role || '').trim().toLowerCase();
    if (goal && goal.proof_review_status === 'Approved' && goalReviewRole === 'manager') {
      initialRatings.leader = Number(goal.proof_review_rating || 0);
    }
  ...
  const setInlineMemberRating = (goalId: number, key: string, rating: number) => {
    setInlineMemberRatingsByGoal((prev) => {
      const current = prev[goalId] || buildInitialRatingsForGoal(goalId);
      return {
        ...prev,
        [goalId]: {
          ...current,
  ...
  const saveInlineMemberRatings = async (goalId: number) => {
    const goal = goals.find(g => Number(g.id) === Number(goalId));
    if (!goal) return;
    ...
    const goalReviewRole = String((goal as any)?.proof_reviewer_role || (goal as any)?.proof_reviewed_role || '').trim().toLowerCase();
    const leaderEligible = goal.proof_review_status === 'Approved' && goalReviewRole === 'manager';
    const ratings = inlineMemberRatingsByGoal[goalId] || buildInitialRatingsForGoal(goalId);

    if (leaderEligible) {
      const leaderRating = Number(ratings.leader || 0);
      if (leaderRating < 1 || leaderRating > 5) {
        window.notify?.('Please rate the team leader before saving', 'error');
        return;
      }
    }

    const unratedMember = submittedTasks.find((t: any) => {
      const value = Number(ratings[`task-${t.id}`] || 0);
      return value < 1 || value > 5;
    });
    if (unratedMember) {
      window.notify?.(`Please rate ${unratedMember.member_name || 'all approved members'} before saving`, 'error');
      return;
    }

    setInlineMemberRatingsSavingGoalId(goalId);
    try {
      const savePromises: Promise<Response>[] = [];

      submittedTasks.forEach((t: any) => {
        const rating = Number(ratings[`task-${t.id}`] || 0);
        savePromises.push(
          fetch(`/api/member-tasks/${t.id}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
      ...
      if (leaderEligible) {
        savePromises.push(
          fetch(`/api/goals/${goalId}`, {
            method: 'PUT',
            headers: getAuthHeaders(),
            body: JSON.stringify({ proof_review_rating: Number(ratings.leader || 0) })
          })
        );
      }
      ...
      if (failedResult) {
        let msg = 'One or more ratings failed to save';
        try { const err = await failedResult.json(); msg = err.error || msg; } catch {}
        throw new Error(msg);
      }

      window.notify?.('Member ratings saved successfully', 'success');
      await Promise.all([
        fetchGoals(),
        refreshProofReviewTasks(goalId),
      ]);
    } catch (e: any) {
      window.notify?.(e?.message || 'Failed to save member ratings', 'error');
    } finally {
      setInlineMemberRatingsSavingGoalId(null);
    }
  };
  ...
  const saveTaskReviewRating = async (taskId: number, goalId: number) => {
    ...
    try {
      ...
      window.notify?.('Manager rating saved', 'success');
      await refreshProofReviewTasks(goalId);
    } catch (e: any) {
      window.notify?.(e?.message || 'Failed to save manager rating', 'error');
    } finally {
      setProofReviewSubmittingTaskId(null);
    }
  };
            ...
            <div className="space-y-3 lg:space-y-0 lg:grid lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-4 lg:items-start">
              ...
              {proofReviewLoadingGoal === proofReviewOpenGoal ? (
                ...
                (() => {
                  ...
                  const reviewRole = String((t as any).reviewer_role || (t as any).proof_reviewed_role || '').trim().toLowerCase();
                  const managerApproved = reviewStatus === 'Approved' && reviewRole === 'manager';
                  const managerRatingLocked = managerApproved && Number(t.proof_review_rating || 0) > 0;
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-5 gap-2.5 items-end">
                                    ...
                                    <div>
                                      ...
                                      <input
                                        ...
                                        max={String(g?.target_date || '').trim().slice(0, 10) || undefined}
                                        className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-2 text-xs"
                                      />
                                    </div>
                                    <div>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <Card>
        <div className="overflow-x-auto pb-1">
          <table className="w-full min-w-[1060px] text-left border-collapse table-fixed text-[13px]">
            <thead><tr className="border-b border-slate-100 dark:border-slate-800">
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-[270px]">Title</th>
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-35">Department</th>
              {activeTab === 'Team' && <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-35">Team</th>}
              {activeTab === 'Individual' && <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-35">Employee</th>}
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-44">Owner</th>
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-20">Priority</th>
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-32">Progress</th>
              <th className="py-2.5 px-2.5 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider w-28">Status</th>
              <th className="py-2.5 px-2 text-[11px] font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wider text-right w-24">Actions</th>
            </tr></thead>
            <tbody>
              {filtered.length === 0 && (
                    ...
                    <tr className={`border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors ${overdue ? 'bg-red-50/30 dark:bg-red-900/5' : ''} ${isArchived ? 'opacity-60' : ''}`}>
                      <td className="py-2.5 px-2.5 align-top">
                        <div className="space-y-0.5 min-w-0">
                          <span className="block min-w-0 truncate font-semibold text-slate-700 dark:text-slate-100 text-[14px]" title={g.title || g.statement}>{g.title || g.statement}</span>
                          <div className="flex flex-wrap items-center gap-1">
                            {overdue && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-100 dark:bg-red-900/40 text-red-600">OVERDUE</span>}
                            {isArchived && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300">ARCHIVED</span>}
                            {g.frequency && g.frequency !== 'One-time' && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400">{g.frequency}</span>}
                          </div>
                      ...
                      <td className="py-2.5 px-2.5 text-sm text-slate-500 dark:text-slate-400 truncate" title={g.department || '\u2014'}>{g.department || '\u2014'}</td>
                      {activeTab === 'Team' && <td className="py-2.5 px-2.5 text-sm text-slate-500 dark:text-slate-400 truncate" title={g.team_name || '\u2014'}>{g.team_name || '\u2014'}</td>}
                      {activeTab === 'Individual' && <td className="py-2.5 px-2.5 text-sm text-slate-500 dark:text-slate-400">
                      ...
                      <td className="py-2.5 px-2.5">
                        {g.delegation ? (
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate min-w-0" title={g.delegation}>
                              {g.delegation}
                            </span>
                          </div>
                        ) : (
                      ...
                      <td className="py-2.5 px-2.5"><span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${priorityColor(g.priority || 'Medium')}`}>{g.priority || 'Medium'}</span></td>
                      <td className="py-2.5 px-2.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm font-bold text-slate-600 dark:text-slate-300 w-10 text-right shrink-0">{g.progress || 0}%</span>
                        </div>
                      </td>
                      <td className="py-2.5 px-2.5 whitespace-nowrap">
                        {isArchived ? (
                          <span className="text-[11px] font-bold uppercase px-2 py-0.5 rounded-full border text-slate-500 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700">Archived</span>
                        ) : (
                        <div className="flex items-center gap-1 whitespace-nowrap">
                          <span className={`text-[11px] font-bold uppercase px-2 py-0.5 rounded-full border ${statusColor(g.status || 'Not Started')}`}>
      ...
      >
        {memberRatingsModal.goalId && (() => {
          ...
          const goalReviewRole = String((goal as any)?.proof_reviewer_role || (goal as any)?.proof_reviewed_role || '').trim().toLowerCase();
          const leaderName = String((goal as any)?.delegation || (goal as any)?.leader_name || (goal as any)?.employee_name || 'Leader').trim() || 'Leader';
          const ratingsLocked = !!goal && goal.proof_review_status === 'Approved' && goalReviewRole === 'manager' && Number(goal.proof_review_rating || 0) >= 1 && Number(goal.proof_review_rating || 0) <= 5 && submittedTasks.every((t: any) => Number(t.proof_review_rating || 0) >= 1 && Number(t.proof_review_rating || 0) <= 5);

          return (
            <div className="space-y-3">
              <div className="rounded-2xl border border-sky-200 dark:border-sky-900/40 bg-sky-50/80 dark:bg-sky-900/15 p-3 space-y-1.5">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-sky-700 dark:text-sky-300">Member Rating Summary</p>
                <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">
                  {ratingsLocked ? 'Ratings are saved and locked.' : 'Approve all proofs first, then rate the leader and every approved member.'}
                </p>
              </div>

              <div className="space-y-3">
                {goal && goal.proof_review_status === 'Approved' && goalReviewRole === 'manager' && (
                  <div className="rounded-2xl border border-emerald-200 dark:border-emerald-900/40 bg-emerald-50/70 dark:bg-emerald-900/15 p-3 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-[11px] font-black uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-300">Leader Rating</p>
                        <p className="truncate text-base font-extrabold text-slate-800 dark:text-slate-100" title={leaderName}>{leaderName}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">This rating locks after save.</p>
                      </div>
                      <div className="shrink-0 rounded-full bg-white/80 dark:bg-slate-900/80 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                        Leader
                      </div>
                    </div>
                    <select
                      ...
                      disabled={memberRatingsModal.submitting || ratingsLocked}
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-base font-bold disabled:opacity-60"
                    >
                ...
                {submittedTasks.map((t: any) => (
                  <div key={`task-rating-${t.id}`} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 space-y-2">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-base font-extrabold text-slate-800 dark:text-slate-100">{t.member_name || `Member #${t.member_employee_id}`}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">{t.title || 'Assigned task'}</p>
                      </div>
                      <select
                        ...
                        disabled={memberRatingsModal.submitting || ratingsLocked}
                        className="w-full max-w-28 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-base font-bold disabled:opacity-60"
                      >
                        <option value="">Rate 1-5</option>
                        {[1, 2, 3, 4, 5].map((r) => (<option key={r} value={r}>{r}/5</option>))}
                      </select>
                    </div>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Approved proof only. Saved ratings are locked.</p>
                  </div>
                ))}
              </div>
