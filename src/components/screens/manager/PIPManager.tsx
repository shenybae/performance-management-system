            ...
            <form className="space-y-4" onSubmit={e => { e.preventDefault(); submitPIP(); }}>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    onChange={v => setPipForm({ ...pipForm, employee_id: String(v) })}
                    placeholder="Select Employee..."
                    dropdownVariant="pills-horizontal"
                  />
                </div>
                <Input label="Start Date" type="date" value={pipForm.start_date} onChange={(e: any) => setPipForm({ ...pipForm, start_date: e.target.value })} required />
                <Input label="End Date" type="date" value={pipForm.end_date} onChange={(e: any) => setPipForm({ ...pipForm, end_date: e.target.value })} min={pipForm.start_date || undefined} required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea rows={2} value={pipForm.deficiency} onChange={e => setPipForm({ ...pipForm, deficiency: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" placeholder="Describe the performance issue..." minLength={10} maxLength={1000} required />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Improvement Objective</label>
                  <textarea rows={2} value={pipForm.improvement_objective} onChange={e => setPipForm({ ...pipForm, improvement_objective: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" placeholder="What must improve..." maxLength={1000} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Action Steps</label>
                  <textarea rows={2} value={pipForm.action_steps} onChange={e => setPipForm({ ...pipForm, action_steps: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" placeholder="Steps to achieve improvement..." maxLength={1000} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Support Provided</label>
                  <textarea rows={2} value={pipForm.support_provided} onChange={e => setPipForm({ ...pipForm, support_provided: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" placeholder="Training, mentoring, etc..." maxLength={1000} />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <Input label="Progress Check Date" type="date" value={pipForm.progress_check_date} onChange={(e: any) => setPipForm({ ...pipForm, progress_check_date: e.target.value })} min={pipForm.start_date || undefined} max={pipForm.end_date || undefined} />
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Supervisor</label>
                  <SearchableSelect options={employees.map(e => ({ value: e.id, label: e.name, avatarUrl: (e as any).profile_picture || null }))} value={employees.find(e => e.name === pipForm.supervisor_name)?.id || ''} onChange={(id: any) => setPipForm({ ...pipForm, supervisor_name: employees.find(e => e.id === id)?.name || '' })} placeholder="Select supervisor..." dropdownVariant="pills-horizontal" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea rows={2} value={pipForm.progress_notes} onChange={e => setPipForm({ ...pipForm, progress_notes: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" placeholder="Notes from check-in meetings..." maxLength={1000} />
              </div>
              <div className="flex justify-end">
                <button type="submit" disabled={!isPIPFormValid()} className="bg-amber-500 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed">{editingId ? 'Update PIP' : 'Create PIP'}</button>
              </div>
            </form>
          </Card>
        </motion.div>
          ...
          <Card>
            ...
            <form className="space-y-4" onSubmit={e => { e.preventDefault(); submitIDP(); }}>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <SearchableSelect
                    ...
                    onChange={v => setIdpForm({ ...idpForm, employee_id: String(v) })}
                    placeholder="Select Employee..."
                    dropdownVariant="pills-horizontal"
                  />
                </div>
                <div>
                  ...
                  <select value={idpForm.status} onChange={e => setIdpForm({ ...idpForm, status: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" required>
              ...
              <div>
                ...
                <select value={idpForm.skill_gap} onChange={e => setIdpForm({ ...idpForm, skill_gap: e.target.value })} className="w-full p-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg text-sm dark:text-slate-100" required>
              ...
              <Input label="Growth Step / Action Plan" value={idpForm.growth_step} onChange={(e: any) => setIdpForm({ ...idpForm, growth_step: e.target.value })} placeholder="Training, mentoring, course, certification..." maxLength={500} required />
              <div className="flex justify-end">
                <button type="submit" disabled={!isIDPFormValid()} className="bg-teal-deep text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-teal-green disabled:opacity-50 disabled:cursor-not-allowed">Create IDP</button>
              </div>
            </form>
          </Card>
        </motion.div>
