...
import { GitBranch, Link2, Shield, Users, ArrowRight, Unlink2, Sparkles } from 'lucide-react';
...
export const LinkedPeople = ({ employees, users, onRefresh }: LinkedPeopleProps) => {
  ...
  const hrAdmins = useMemo(() => (Array.isArray(users) ? users : []).filter((u: any) => normalize(u?.role) === 'hr'), [users]);
  const employeeMap = useMemo(() => {
    const map = new Map<number, Employee>();
    (employees || []).forEach((person) => map.set(Number(person.id), person));
    return map;
  }, [employees]);
  ...
  const sortedEmployees = useMemo(() => [...(employees || [])].sort((a, b) => personLabel(a).localeCompare(personLabel(b))), [employees]);
  const [selectedEmployeeId, setSelectedEmployeeId] = useState<number | ''>(sortedEmployees[0]?.id || '');
  ...
  const selectOptions = useMemo(() => {
    return sortedEmployees.map((person) => ({
      value: person.id,
      label: personLabel(person),
      avatarUrl: (person as any).profile_picture || null,
    }));
  }, [sortedEmployees]);
      ...
      <div key={person.id} className={depth > 0 ? 'pl-6 border-l border-dashed border-slate-200 dark:border-slate-700 ml-3' : ''}>
        <motion.button
          ...
          onClick={() => {
            ...
            setLinkedToId((person as any).manager_id || '');
          }}
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 truncate">{(person as any).position || 'No position'} • {(person as any).dept || 'No department'}</p>
            </div>
            <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-400">
              <GitBranch size={12} />
              <span>{descendants.length} linked</span>
            </div>
          </div>
        </motion.button>
  ...
  const saveLink = async (managerId: number | null) => {
    ...
    const targetLabel = managerId ? personLabel(employeeMap.get(managerId)) : 'remove the existing link';
    if (!(await appConfirm(
      managerId ? `Link ${personLabel(selectedEmployee)} to ${targetLabel}?` : `Unlink ${personLabel(selectedEmployee)} from their current manager?`,
      { title: 'Update Link', confirmText: managerId ? 'Link' : 'Unlink', icon: 'success' }
    ))) return;
    ...
    const payload = {
      ...
      dept: (selectedEmployee as any).dept || '',
    ...
    try {
      ...
      window.notify?.(managerId ? 'Linked people updated' : 'Link removed', 'success');
      onRefresh();
    } catch {
  ...
  const linkedCount = employees.filter((person) => Number((person as any).manager_id || 0) > 0).length;
  const managerCount = employees.filter((person) => roleLabel(person) === 'Manager').length;
  const supervisorCount = employees.filter((person) => roleLabel(person) === 'Supervisor').length;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <SectionHeader
        title="Linked People"
        subtitle="Visualize who reports to whom and update links from a single HR-only screen."
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[380px_minmax(0,1fr)] gap-6 items-start">
        <div className="space-y-6">
          <Card>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Hierarchy Snapshot</h3>
            </div>
          ...
          <Card>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Link Editor</h3>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Select person</label>
                <SearchableSelect
                  ...
                  onChange={(value) => {
                    ...
                    const nextPerson = employeeMap.get(nextId);
                    setLinkedToId(nextPerson && (nextPerson as any).manager_id ? Number((nextPerson as any).manager_id) : '');
                  }}
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Reports to</label>
                <SearchableSelect
                  ...
                  placeholder="Assign a manager or supervisor..."
                  searchable
                  allowEmpty
                  emptyLabel="No link"
                  dropdownVariant="modal"
          ...
          {selectedEmployee && (
            <Card>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Selected Chain</h3>
              </div>
                ...
                {chain.map((person, index) => (
                  <div key={person.id} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{roleLabel(person)} • {(person as any).dept || 'No department'}</p>
                    </div>
        ...
        <Card className="min-h-[72vh]">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
            <div>
              <h3 className="text-lg font-black text-slate-900 dark:text-slate-100">Linked Organization View</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Click a person to focus their chain and change the reporting link.</p>
            </div>
          ...
          <div className="space-y-4">
            {filteredRoots.length > 0 ? filteredRoots.map((person) => renderNode(person)) : (
              <div className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 p-8 text-center text-slate-500 dark:text-slate-400">
                No matching linked people found.
              </div>
            )}
          </div>
        </Card>
      ...
      {selectedEmployee && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card className="lg:col-span-1">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{(selectedEmployee as any).position || 'No position'} • {(selectedEmployee as any).dept || 'No department'}</p>
          </Card>
          <Card className="lg:col-span-1">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{selectedManager ? `${roleLabel(selectedManager)} • ${(selectedManager as any).dept || 'No department'}` : 'Use the link editor to assign one.'}</p>
          </Card>
          <Card className="lg:col-span-1">
