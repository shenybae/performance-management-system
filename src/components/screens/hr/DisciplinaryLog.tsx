...
export const DisciplinaryLog = ({ employees, currentUser }: DisciplinaryLogProps) => {
  ...
  const [recordFilter, setRecordFilter] = useState<'archived' | 'acknowledged' | 'pending'>('pending');
...
<style>body{font-family:Arial,sans-serif;font-size:12px;line-height:1.35;color:#111;margin:24px 32px}h1{font-size:17px;font-weight:900;margin-bottom:2px}h2{font-size:13px;font-weight:700;margin:16px 0 6px;border-bottom:1px solid #ccc;padding-bottom:3px}table{width:100%;border-collapse:collapse;margin-bottom:8px;table-layout:fixed}td,th{padding:5px 8px;border:1px solid #ccc;font-size:11px;vertical-align:top;overflow-wrap:anywhere;word-break:break-word}th{background:#f3f4f6;font-weight:700;text-align:left}.label{font-weight:700;width:30%}.sig-row{display:flex;flex-wrap:wrap;gap:16px;margin-top:16px;align-items:flex-start}.sig-row>div{flex:1 1 220px;min-width:0;page-break-inside:avoid}.badge{display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:#dcfce7;color:#166534}@media print{body{margin:14px 20px}}</style></head>
<body>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => exportToCSV(filteredRecords, 'discipline_records')} className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
        ...
        <Card>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
            ...
            <div className="flex w-full sm:w-auto items-center gap-2">
              <select value={recordFilter} onChange={(e) => setRecordFilter(e.target.value as 'archived' | 'acknowledged' | 'pending')} className="px-3 py-2 rounded-lg text-sm font-bold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 shrink-0">
            ...
            <table className="w-full min-w-[1180px] table-fixed text-sm">
              ...
              <tbody>
                ...
                {filteredRecords.length === 0 && (
                  <tr>
                    <td colSpan={11} className="py-10 text-center text-slate-400">No records match the selected filters.</td>
                  </tr>
                )}
              </tbody>
            </table>
