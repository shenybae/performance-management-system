...
export function sigBlockHtml(sig: string | null, label: string, date: string | null, printedName?: string, minWidth = 160) {
  ...
  return `
    <div style="flex:1;text-align:center;min-width:${minWidth}px;">
      ...
      <div style="min-height:52px;display:flex;align-items:center;justify-content:center;margin-bottom:2px;">
        ${signatureSrc ? `<img src="${signatureSrc}" style="max-height:50px;max-width:100%;object-fit:contain;display:block;margin:0 auto;" />` : '<div style="width:80%;height:1px;border-bottom:1px solid #000;"></div>'}
      </div>
